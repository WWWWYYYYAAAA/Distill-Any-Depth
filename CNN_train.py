...
def train(model, loss_func, optimizer, checkpoints, epoch):
    ...
    for epoch in range(0, epoch):
        ...
        #         val_acc_epoch))
        print(
            'epoch:{} | time:{:.4f} | train_loss:{:.4f} | train_acc:{:.4f}'.format(
                epoch,
                end_time - start_time,
                train_loss_epoch,
                train_acc_epoch))
        ...
        print('Best loss for Validation :{:.4f} at epoch {:d}'.format(best_loss, best_epoch))
        # 每迭代50次保存一次模型
        # if epoch % 50 == 0:
